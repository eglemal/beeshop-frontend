...
describe("TopBar", () => {
  describe("Layout", () => {
    it("has application logo", () => {
      ...
      expect(image.src).toContain("eshoplogo.jpg");
    });
    ...
    it("has link to register", () => {
      ...
      const signupLink = queryByText("Register");
    ...
    it("has link to user cart when user is logged in", () => {
      const { queryByText } = setup(loggedInState);
      const profileLink = queryByText("My Cart");
  ...
  describe("Interactions", () => {
    it("displays the login and register links when user clicks logout", () => {
      const { queryByText } = setup(loggedInState);
      const logoutLink = queryByText("Logout");
  ...
  //       store.dispatch(authActions.logginSuccess(loggedInState));
